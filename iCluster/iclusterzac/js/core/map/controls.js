define(['OpenLayers','mapStyles','features','marker','georeference','popup','poi','modal','config','dataSource','request','notification','cluster'],function(OL,mapStyle,Features,Marker,Georeference,Popup,Poi,Modal,config,dataSource,Request,Notification,Cluster) {
	...
	var g = Map.getLayer('Georeference');
		...
		sel_circ: new (getClickEvent(function(e){
			radio=document.getElementById("val_radio").value;        
			var lonlat = Map.map.getLonLatFromViewPortPx(e.xy);          
			document.getElementById('coords').value="";			
			var polygon = createGeodesicPolygon(lonlat, radio, 50, 0, new OpenLayers.Projection("EPSG:900913"));			
			var circleFeature = new OpenLayers.Feature.Vector(polygon); 
			var arr = Map.map.getLayersByName("A_influencia");
                if (arr.length) {
                    Map.map.removeLayer(vectors);                    
                }
			vectors = new OpenLayers.Layer.Vector("A_influencia", {
			...
			Map.map.addLayer(vectors);			
			vectors.addFeatures(circleFeature);	
			},Map)
			
		),
		sel_poly: new c.Measure(		
                    h.Polygon, {
                        persist: true,
						immediate:false,
                        handlerOptions: {
                            layerOptions: {renderers: r,styleMap:  new OpenLayers.StyleMap({
								fillColor: "#336699",
								fillOpacity: 0.4, 
								strokeColor: "#003366",
								strokeOpacity: 0.8,
	...
	*/
	...
	if (document.getElementById("poligono")){
        ...
        for (var i = 0; i <= vertices.length - 1; i++) {
            var x = vertices[i].x;
		    var y = vertices[i].y;
		    a=new OpenLayers.LonLat(x, y).transform(Map.map.getProjectionObject(),new OpenLayers.Projection("EPSG:4326"))
		    if (i==0){
                paso=a.lon.toString() + " " + a.lat.toString();
				var arr = Map.map.getLayersByName("A_influencia");
						if (arr.length) {
							Map.map.removeLayer(vectors);                    
						}	
		    ...
		    if (i==(vertices.length - 1)){
                document.getElementById('coords').value=document.getElementById('coords').value + a.lon.toString() + " " + a.lat.toString()+ "," + paso; 
            }else{
				document.getElementById('coords').value=document.getElementById('coords').value + a.lon.toString() + " " + a.lat.toString()+ ",";
            }
        }
	}else{
        var geometry = event.geometry;
