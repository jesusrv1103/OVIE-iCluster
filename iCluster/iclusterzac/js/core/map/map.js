...
define(['OpenLayers','config','mapControls','mapStyles','tree','mapLayers','mapTree','features','wps','marker','popup','georeference','linetime','poi','notification','escuelas','modal','help','request','tutorial','thirdService','geolocation','cluster','dataSource','routing','toolsConfig','printControl'],function(OL,config,ctl,mapStyle,tree,Layer,Tree,Features,wps,Marker,Popup,Georeference,LineTime,Poi,Notification,Escuelas,Modal,Help,Request,Tutorial,TS,Geolocation,Cluster,DataSource,Routing,toolsConfig,printControl) {
    ...
    Map.getWidth = getWidth;
		...
		var elimina_tema = function(){
	    var arr = Map.map.getLayersByName("Tema");
						if (arr.length) {
							Map.map.removeLayer(vectors);                    
						}	
		OpenLayers.Util.getElement('paleta').style.display='none';
        return null;
		
    };
	Map.elimina_tema = elimina_tema;
	
	
	
	var tematizar = function(){
	    var iframe= document.getElementById('datos');
		//alert(iframe.contentWindow.document.getElementById('grupo').value);			
		if ((iframe.contentWindow.document.getElementById('grupo').value == '') || (iframe.contentWindow.document.getElementById('variable').value=='')){
								alert("Debe seleccionar las variables a mostrar");
				}else{
				//document.getElementById("loading").style.display='';
				var mensaje =Notification.show({message:'Hay va'});
				var grupo=iframe.contentWindow.document.getElementById('grupo').value;
				var variable=iframe.contentWindow.document.getElementById('variable').value;
				variable=variable.trim();
				var tit= iframe.contentWindow.document.getElementById('variable').options[iframe.contentWindow.document.getElementById('variable').selectedIndex].text;
				var cluster= iframe.contentWindow.document.getElementById('grupo').options[iframe.contentWindow.document.getElementById('grupo').selectedIndex].text;
				var clusterid=iframe.contentWindow.document.getElementById('grupo').value;								
				var tema = iframe.contentWindow.document.getElementById('tema').value;
				
                var arr = Map.map.getLayersByName("Tema");
				if (arr.length) {
						Map.map.removeLayer(arr[0]);
				}
				var xmlhttp=new XMLHttpRequest();
				xmlhttp.onreadystatechange=function() {
					if (xmlhttp.readyState==4 && xmlhttp.status==200) {
						var datos_json=xmlhttp.responseText;
						//datos_json=
						variables=datos_json.substring(0,datos_json.indexOf("{"));
						//alert( datos_json);
						varia=variables.split(",");						
						datos_json=datos_json.substr(datos_json.indexOf("{"));
						t_nac_clases=varia[3];
						t_nac=varia[4];
						var geojson_format = new OpenLayers.Format.GeoJSON();
						datos =geojson_format.read(datos_json);
						if (variable == 'c_loca'){
							min=1000000;
							max=-1000000;						
							for (var i = 0; i < datos.length; i++) {
								//alert(vector_layer.features[i].attributes.dato);
								divi=(Number(datos[i].attributes.dato)/Number(datos[i].attributes.ptotal));
								coef=divi/(Number(t_nac_clases)/Number(t_nac)); 
								if (min > coef){
									min = coef;
								}
								if (max < coef){
								...
								datos[i].attributes.dato=coef;
							}
							min=Math.round(min * 100) / 100;
							max=Math.round(max * 100) / 100;
							rango=(max - min)/10;
							actualiza_tema(min,rango);
							OpenLayers.Util.getElement('minimo').innerHTML=numerocomas(min);
							OpenLayers.Util.getElement('maximo').innerHTML=numerocomas(max);
							OpenLayers.Util.getElement('paleta').style.display='';
						
						}else{
							actualiza_tema(varia[0],varia[1]);
							OpenLayers.Util.getElement('paleta').style.display='';
							OpenLayers.Util.getElement('minimo').innerHTML=numerocomas(varia[0]);
							OpenLayers.Util.getElement('maximo').innerHTML=numerocomas(varia[2]);
							//datos=geojson_format.read(datos_json);
							
						}
						
						
						var vector_layer = new OpenLayers.Layer.Vector("Tema",
						{
                        styleMap: new OpenLayers.StyleMap(styletheme)
						}
						);
						vector_layer.events.register('beforefeaturesadded','',loaded);
						Map.map.addLayer(vector_layer);
						vector_layer.addFeatures(datos);
						
						
						mensaje.hide();
					}				       
				}				
				//xmlhttp.open("GET","php/visualiza_geo.php?grupo="+grupo+"&variable="+variable+"&tema="+tema,true);				
				//xmlhttp.send();
        ...
        callbacks: {
		      over: function(feature) {
                //tooltip = new OpenLayers.Popup.Anchored('tooltip');
                //tooltip.displayClass = "olpopupwindow";
                //tooltip.lonlat = feature.geometry.getBounds().getCenterLonLat();
                //tooltip.size = new OpenLayers.Size(20,20);
                //tooltip.contentHTML = "<div style='font-size:.8em;'><center>" + feature.attributes.nom_ent + "<br><br>"+ feature.attributes.dato+"</center></div>";
                //tooltip.backgroundColor= "#808080";
                //tooltip.opacity = 0.9;
                //tooltip.autoSize = true;
				
				if (feature.attributes.dato==0){
					dato='*';
				}else{
					dato=feature.attributes.dato;
					divi=(Number(dato)/Number(feature.attributes.ptotal));
					dato=numerocomas(dato);
					
				}
				//alert(feature.attributes.ptotal);
				if ((variable == 'ue') || (variable == 'p_ocupado')){
					OpenLayers.Util.getElement('info').innerHTML="<center><h3 style='background: #49A3B1; color:white; padding:0px;margin: 0px;'>" + feature.attributes.entidad+ " "+anio+"</h3><h3 style='padding:0px;margin: 5px;'>"+cluster+"</h3><h3 style='padding:0px;margin: 0px; font-weight: normal;'><i>" + tit + "</i></h3><h2 style='margin:15px;padding:0px;font-family: Arial;font-weight:normal;'>"+ dato +"</h2></center>";
				}else{
					if (variable == 'c_loca'){
							OpenLayers.Util.getElement('info').innerHTML="<center><h3 style='background: #49A3B1; color:white; padding:0px;margin: 0px;'>" + feature.attributes.entidad+ " "+anio+"</h3><h3 style='padding:0px;margin: 5px;'>"+cluster+"</h3><h3 style='padding:0px;margin: 0px; font-weight: normal;'><i>" + tit + "</i></h3><h2 style='margin:15px;padding:0px;font-family: Arial;font-weight:normal;'>"+ dato +"</h2></center>";						
					}else{
					if ((variable != 't_gastos') && (variable != 't_ingresos')){
						OpenLayers.Util.getElement('info').innerHTML="<center><h3 style='background: #49A3B1; color:white; padding:0px;margin: 0px;'>" + feature.attributes.entidad+ " "+anio+"</h3><h3 style='padding:0px;margin: 5px;'>"+cluster+"</h3><h3 style='padding:0px;margin: 0px;font-weight: normal;'><i>" + tit + "</i></h3><h2 style='margin:5px;padding:0px;font-family: Arial;font-weight:normal;'>"+ dato +"<span style='font-size:8px'>*"+idioma[58]+"</span></h2></center>";
						}else{
						OpenLayers.Util.getElement('info').innerHTML="<center><h3 style='background: #49A3B1; color:white; padding:0px;margin: 0px;'>" + feature.attributes.entidad+ " "+anio+"</h3><h3 style='padding:0px;margin: 5px;'>"+cluster+"</h3><h4 style='padding:0px;margin: 0px;font-weight: normal;'><i>" + tit + "</i></h4><h2 style='margin:5px;padding:0px;font-family: Arial;font-weight:normal;'>"+ dato +"<span style='font-size:8px'>*"+idioma[58]+"</span></h2></center>";
						}
					}
				}	
				OpenLayers.Util.getElement('info').style.display='';
                
            },
            out: function(feature) {
               //map.removePopup(tooltip);
               // tooltip.destroy();
               // tooltip = null;				
				OpenLayers.Util.getElement('info').style.display='none';
            }
        },        
        eventListeners: {
            featurehighlighted: function(event) {
				OpenLayers.Util.getElement('info_estadof').src="noreste/muestra_info.php?ent="+event.feature.attributes.ent+"&cluster="+clusterid+"&idioma="+idioma[18]+"&anio="+anio;
				//OpenLayers.Util.getElement('info_estado').innerHTML=HTML;
				OpenLayers.Util.getElement('info_estado').style.display='';
				
            },
            featureunhighlighted: function(feat) {
                OpenLayers.Util.getElement('info_estado').style.display='none';
            }
        }    
    });    
     selectControl.handlers['feature'].stopDown = false;
    selectControl.handlers['feature'].stopUp = false;
    Map.map.addControls([selectControl]);
	...
	Map.tematizar = tematizar;
	
	var elimina_vec = function(){
	    var arr = Map.map.getLayersByName("A_influencia");
						if (arr.length) {
							Map.map.removeLayer(vectors);                    
						}	
        return null;
    };
	Map.elimina_vec = elimina_vec;
	
	
	
	var cargaGrupo = function(){		
		var iframe= document.getElementById('datos');
		var id = iframe.contentWindow.document.getElementById('grupo').value;
		var tema = iframe.contentWindow.document.getElementById('tema').value;
		var tipo = iframe.contentWindow.document.getElementById('grupo').options[iframe.contentWindow.document.getElementById('grupo').selectedIndex].getAttribute('tipo')
		var imagen = iframe.contentWindow.document.getElementById('grupo').options[iframe.contentWindow.document.getElementById('grupo').selectedIndex].getAttribute('imagen')
		var nombre = iframe.contentWindow.document.getElementById('grupo').options[iframe.contentWindow.document.getElementById('grupo').selectedIndex].getAttribute('nombre')								
		console.log(tema);
		var xmlhttp=new XMLHttpRequest();
				...
				xmlhttp.onreadystatechange=function() {
					if (xmlhttp.readyState==4 && xmlhttp.status==200) {
						var datos_json=xmlhttp.responseText;
						//console.log(nombre);
						if (datos_json.length < 50){
							alert('No existe información para el grupo seleccionado.');	
						}else{
							var geojson_format = new OpenLayers.Format.GeoJSON();						
							datos =geojson_format.read(datos_json);							
							if (tema=='1'){
							var s = new OpenLayers.Style({'pointRadius': 10,'externalGraphic': './img/grupos/'+imagen+'.png'});
							nombrec="Puntos"+imagen;
							}else{
							var s = new OpenLayers.Style({'pointRadius': 10,'externalGraphic': './img/clusters/'+imagen+'.png'});
							nombrec="PuntosC"+imagen;
							}
							
							var vector_layer = new OpenLayers.Layer.Vector(nombrec,{styleMap: s});
							
							vector_layer.events.register('beforefeaturesadded','',loaded);
							Map.map.addLayer(vector_layer);
							vector_layer.id=nombre;
		...
		if (tema==1){
			var arr = smap.getLayersByName("Puntos"+imagen);
			if (arr.length == 0){
				xmlhttp.open("GET","php/consultar_grupo.php?id="+id+"&tipo="+tipo+"&tema="+tema,true);
				xmlhttp.send();	
			}else{
				alert("Este grupo ya esta visible");
			}
		}else{
			var arr = smap.getLayersByName("PuntosC"+imagen);
			if (arr.length == 0){
				xmlhttp.open("GET","php/consultar_grupo.php?id="+id+"&tipo="+tipo+"&tema="+tema,true);
				xmlhttp.send();	
			}else{
		...
		function loaded(){	
			var arr = Map.map.getLayersByClass("OpenLayers.Layer.Vector");
			arrn=[];
			console.log(arr.length);
			for (i=0;i<arr.length;i++){
				if (arr[i].name.search("Puntos")!=-1){
				arrn.push(arr[i]);
				}
			}	
			//console.log(arrn);
        ...
        var params = {};
        params['title']=f.layer.id;
		params['element']='feature';        
        params['prev']=f.attributes.nom_estab;                
		console.log(p);
		p.y=p.y+25;        
		console.log(p);
		params['px']=p;
        ...
        Popup.show(params);
			...
			var selectControl = new OpenLayers.Control.SelectFeature(arrn, {	 			  
                        onSelect:selected,
	...
	Map.cargaGrupo = cargaGrupo;
   
	
	var borraGrupo = function(){
		var iframe= document.getElementById('datos');
		var id = iframe.contentWindow.document.getElementById('grupo').value;
		var tipo = iframe.contentWindow.document.getElementById('grupo').options[iframe.contentWindow.document.getElementById('grupo').selectedIndex].getAttribute('tipo')
		var imagen = iframe.contentWindow.document.getElementById('grupo').options[iframe.contentWindow.document.getElementById('grupo').selectedIndex].getAttribute('imagen')
		var nombre = iframe.contentWindow.document.getElementById('grupo').options[iframe.contentWindow.document.getElementById('grupo').selectedIndex].getAttribute('nombre')	
		nombrec="Puntos"+imagen;		
		var arr = Map.map.getLayersByName(nombrec);				
		if (arr.length) {
			Map.map.removeLayer(arr[0]);                    			
		}	
		return null;
    };
	Map.borraGrupo = borraGrupo;
    ...
    */
    ...
    return{
        ...
        goCoords:goCoords,
        getWidth:getWidth,
		elimina_vec:elimina_vec,
		tematizar:tematizar,
		elimina_tema:elimina_tema,
		cargaGrupo:cargaGrupo,
		borraGrupo:borraGrupo,
