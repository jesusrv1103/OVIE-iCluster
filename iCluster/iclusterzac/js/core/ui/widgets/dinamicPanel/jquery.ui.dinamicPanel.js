...
$.widget("custom.dinamicPanel", {
     ...
     panels:[{id:'searchPanel',type:'search',height:180,label:'Resultado',desc:'Resultados de b&uacute;squeda',icon:'dinamicPanel-sprite dinamicPanel-search-big'},
			 ...
			 {id:'herramienta',type:'etools',height:230,label:'Herr.',desc:'Herramientas', icon:'dinamicPanel-sprite dinamicPanel-herramientas-big'},
             {id:'agrupa',type:'eagrupa',height:520,label:'Agrup.',desc:'Agrupamientos Industriales',icon:'dinamicPanel-sprite dinamicPanel-agrupa-big'}
             ],
     lastSearch:'',
     id:'',
     searchTimer:0,
     ...
     createPanel:function(type,data){
		...
		//		});
			...
			case 'etools':	
				umap=obj.options.map;
				var obj = this;				
				//obj.options.map.activeCtl({control:"sel_poly",active:true});
				//obj.options.map.activeCtl({control:"sel_poly",active:false});
				cadena='';
				...
				cadena+= '<td width="25%" align="center"><span id="'+obj.id+'_asel" style="cursor:pointer" title="Crear área de selección" class="dinamicPanel-tabIcon dinamicPanel-sprite dinamicPanel-seleccion-big"></span></td> '
				cadena+= '<td width="25%" align="center"><span id="'+obj.id+'_acon" style="cursor:pointer" title="Datos en el área de selección" class="dinamicPanel-tabIcon dinamicPanel-sprite dinamicPanel-consulta-big"></span></td>'
				//cadena+= '<td width="25%" align="center"><span id="'+obj.id+'_dgeo" style="cursor:pointer" class="dinamicPanel-tabIcon dinamicPanel-sprite dinamicPanel-domgeo-big"></span></td>'
				cadena+= '</tr>'
				cadena+= '<tr>'
				cadena+= '<td align="center" id="tdstreet"><span class="dinamicPanel-tabDescription">Imágenes de calle</span></td>'
				cadena+= '<td colspan="2" align="center" id="tdselecc"><span class="dinamicPanel-tabDescription">Selección transversal</span></td> '				
				cadena+= '</tr>'
				cadena+= '</table>';						  
				cadena+= '</center></div><br>';
				cadena+= '<div id="asel" style="display:none"><center><table>';
				cadena+= '<tr>';
                cadena+= '<td><input type="radio" name="tipos" id="'+obj.id+'_radio" value="1" name="radio"/><label>Radio: </label><input name="texto" id="val_radio" type="text" value="500" style="width: 40px" /> m.</td>';
                cadena+= '<td><input type="radio" name="tipos" id="poligono" value="2"/><label> Polígono</label></td>';				
				cadena+= '</tr>';
				cadena+= '</table></center></div>';				
				$('#'+obj.id+'_herramienta').html(cadena);
				document.getElementById('tdstreet').style.backgroundColor = "";
				document.getElementById('tdselecc').style.backgroundColor = "";					
					$("#"+obj.id+'_radio').click(function(){                												
						var list = obj.geo;   //listado mediciones
						var map = obj.options.map;
						document.f1.coords.value = "";
						obj.options.map.elimina_vec();
						map.activeCtl({control:"sel_circ",active:true});
					});								
					...
					$("#poligono").click(function(){                												
						...
						document.f1.coords.value = "";
						obj.options.map.elimina_vec();
						map.activeCtl({control:"sel_poly",active:true});						
					});
					
					$("#"+obj.id+'_acon').click(function(){     
						if (document.f1.coords.value == ""){
							 msg = obj.options.map.Notification({message:'No existe área de selección. Verifique...',time:2000});
						}else{
						document.f1.submit();
					...
					$("#"+obj.id+'_point_btnstreet').click(function(){                						
						document.getElementById('asel').style.display="none";						
						document.getElementById('tdstreet').style.backgroundColor = "#13679F"; 
						document.getElementById('tdselecc').style.backgroundColor = "";						
						var list = obj.geo;   //listado mediciones
						var map = obj.options.map;
						//obj.options.map.elimina_vec();
					...
					$("#"+obj.id+'_asel').click(function(){                						
						document.getElementById('asel').style.display="";        
						document.getElementById('tdstreet').style.backgroundColor = "";
						document.getElementById('tdselecc').style.backgroundColor = "#13679F";
						$("#"+obj.id+'_radio').click();
						});	
						
					$("#"+obj.id+'_point_btnstreet').click();
			break;	
			case 'edit':
                    var cadena = '';
